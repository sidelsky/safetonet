...
class PLL_Bulk_Translate {
					...
					foreach ( $post_ids as $post_id ) {
						foreach ( $data['pll-translate-lang'] as $lang ) {
							if ( 'sync' !== $data['translate'] ) {
								$this->sync_post->save_group( $post_id, array() );
							}

							if ( ! $this->model->post->get_translation( $post_id, $lang ) ) {
								$this->sync_post->copy_post( $post_id, $lang, 'sync' === $data['translate'] );
								$done++;
							} else {
								$missed++;
							}
			...
			if ( ! empty( $data['media'] ) ) {
				...
				if ( ! empty( $post_ids ) ) {
					foreach ( $post_ids as $post_id ) {
						foreach ( $data['pll-translate-lang'] as $lang ) {
							if ( ! $this->model->post->get_translation( $post_id, $lang ) ) {
								$this->filters_media->create_media_translation( $post_id, $lang );
								$done++;
							} else {
								$missed++;
							}
	...
	public function admin_notices() {
		if ( isset( $_GET['translated'] ) ) {
			...
			if ( $missed = (int) $_GET['missed'] ) {
				add_settings_error(
					'general',
					'settings-updated',
					sprintf(
						/* translators: %d is a number of posts */
						_n( '%d translation was not created to avoid overwriting content.', '%d translations were not created to avoid overwriting content.', $missed, 'polylang-pro' ),
						$missed
					),
					'notice-warning'
				);
